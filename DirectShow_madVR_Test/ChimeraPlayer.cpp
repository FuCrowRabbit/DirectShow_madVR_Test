...
auto ChimeraPlayer::ready() noexcept(false) -> void
{
   this->readyVideo();
}

auto ChimeraPlayer::ready(const wchar_t* path) noexcept(false) -> void
{
   this->setVideoPath(path);
   this->ready();
}

auto ChimeraPlayer::play() const noexcept(false) -> void
{
...
auto ChimeraPlayer::readyVideo() noexcept(false) -> void
{
   ...
   check(this->graph_builder_->QueryInterface(IID_IBasicVideo2, reinterpret_cast<void**>(&this->basic_video2_)));
   check(this->graph_builder_->QueryInterface(IID_IVideoWindow, reinterpret_cast<void**>(&this->video_window_)));
   ...
   check(this->graph_builder_->AddSourceFilter(this->now_playing_path_, this->now_playing_path_, &source));
   check(this->capture_graph_builder2_->RenderStream(nullptr, &MEDIATYPE_Video, source, nullptr, nullptr));
   check(this->capture_graph_builder2_->RenderStream(nullptr, &MEDIATYPE_Audio, source, nullptr, nullptr));
}
